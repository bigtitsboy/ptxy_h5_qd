<template>
  ...
  >
    <div class="outBox" ref="addGoods_outBox">
      ...
      <group>
        <x-input style="padding: 10px 0" title="物品标题" placeholder="请输入物品标题" v-model="goodTitle"></x-input>
      </group>
...
export default {
  ...
  methods: {
    ...
    addGood() {
      ...
      this.form = {
        lostName: this.goodTitle,
        lostContent: this.goodsDetail,
        remark: this.checkSort[0],
        phonenumber: this.phonenumber.replace(/\ +/g, ''),
        lostFoundDXImageList: activityInfoImageList
      }
      this.$func.axios(this.$api.addFound, this.form, {
        type: 'POST',
        openLoad: true,
        closeLoad: true,
        flag: 1
